...
public class ShadowVolume : MonoBehaviour
{
	...
	public Material stencilDrawIntersection, stencilVolumeCast;
	...
	protected void DrawShadowMesh()
	{
		...
		if (shadowVolumeRenderer == null || !shadowVolumeRenderer.enabled || !shadowVolumeRenderer.gameObject.activeInHierarchy )
		{
			return;
		}
		...
		if (backend == ShadowVolumeBackend.StencilBuffer)
		{
			if (isSimple)
			{
				...
				Graphics.DrawMesh(shadowMesh, shadowTransform, stencilDrawIntersection, layer, null, 0, null, false, false);
				if ( shadowVolumeRenderer.shadowCastAlpha != 0f ) {					
					Graphics.DrawMesh(shadowMesh, shadowTransform, stencilVolumeCast, layer, null, 0, null, false, false);
				}
			}
			else
			{
	...
	{
		//shadowMesh.bounds.Expand( 1.0f * Time.deltaTime );
		Gizmos.color = new Color( 0.0f, 0.0f, 1.0f, 0.2f );
		Gizmos.DrawCube( transform.position + shadowMesh.bounds.center, shadowMesh.bounds.size );
	}
	*/
